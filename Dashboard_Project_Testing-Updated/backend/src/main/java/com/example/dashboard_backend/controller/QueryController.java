...
public class QueryController {
     ...
     *
     * <p>Turning a raw query config into an execution-ready one (dataset resolution, field remap and the
     * join-aware {@code datasetFromSql} derived table) is delegated to {@link QueryConfigNormalizer} so
     * that this controller and {@code DashboardService} share the exact same normalization — including
     * the join logic.
     */
    ...
    public QueryController(JdbcTemplate jdbcTemplate, QueryConfigNormalizer configNormalizer) {
    ...
    public static GeneratedQuery generateSql(JsonNode config) {
        ...
        // 3. FROM
        // When the dataset has nested child tables, the normalizer supplies a pre-built flattened
    ...
    public Map<String, String> generateQuery(@org.springframework.web.bind.annotation.RequestBody JsonNode config) {
        JsonNode normalizedConfig = configNormalizer.normalize(config);
