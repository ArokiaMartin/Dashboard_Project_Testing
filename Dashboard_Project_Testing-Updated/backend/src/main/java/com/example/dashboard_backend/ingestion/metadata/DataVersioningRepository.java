...
public class DataVersioningRepository {
    ...
    public Optional<DataVersion> findByChecksumAndSchema(String schemaId, String checksum) {
        return jdbcTemplate.query(
                "SELECT * FROM data_versions WHERE schema_id = ? AND checksum = ?",
    ...
    public List<DataVersion> getVersionsBySchema(String schemaId) {
        return jdbcTemplate.query(
                "SELECT * FROM data_versions WHERE schema_id = ? ORDER BY version_number ASC",
                this::mapRowToDataVersion,
                schemaId
        );
    }
     ...
     *
     * @return the number of rows removed (0 when the version id did not exist).
     */
    ...
    private int getNextVersionNumber(String schemaId) {
        List<Integer> result = jdbcTemplate.query(
                "SELECT COALESCE(MAX(version_number), 0) + 1 FROM data_versions WHERE schema_id = ?",
