...
import { DataTable } from '@shared/models';
...
export class UploadService {
  ...
  /**
   * Commits a staged JSON upload so it persists as a queryable dataset. The backend `/upload`
   * endpoint only *analyzes* JSON (returning an uploadToken) and stages it in memory; CSV is already
   * ingested at that point. This confirms the staged JSON so it shows up in the datasets list. The
   * backend sanitizes and de-duplicates the table name, so re-uploading the same file is safe.
   */
  private async ingestStagedUpload(uploadToken: string, file: File): Promise<void> {
    await this.http.post(`${environment.apiUrl}/data/ingest`, {
      uploadToken,
      tableName: this.baseName(file.name),
   ...
   */
  getChecksum(): string {
    if (this.lastChecksum) return this.lastChecksum;
    if (!this.tables.length) return '';
    const tableData = this.tables.map(t => ({
      name: t.name,
      columns: t.columns,
      rows: t.rows
    }));
    this.lastChecksum = this.versioningService.calculateChecksum(tableData);
