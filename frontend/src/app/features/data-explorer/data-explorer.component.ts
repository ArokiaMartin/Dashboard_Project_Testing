    ...
    <div class="page" [class.fullscreen]="fullscreen">
      ...
      <div class="head" *ngIf="!fullscreen">
        <div>
          ...
          <p class="sub" *ngIf="!loading && activeDataset">{{ activeDataset.original_filename }} — {{ userId }}</p>
...
export class DataExplorerComponent implements OnInit, OnDestroy {
  ...
  loadDatasets() {
    ...
    this.http.get<Dataset[]>(`${environment.apiUrl}/datasets`).subscribe({
      next: (data) => {
        // Surface only the active dataset (schema). Its latest version is shown; older
        // versions and other datasets remain in the database but are not linked here.
        const all = data || [];
        const scoped = all.filter((d) => this.active.datasetMatchesActive(d));
        const pick = scoped.length ? [scoped[0]] : [];
        this.datasets = pick;
        this.loading = false;
