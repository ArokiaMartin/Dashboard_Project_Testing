...
export class SchemaDataUploadComponent implements OnInit, OnDestroy {
  ...
  private sendIngest(data: any): void {
    this.schemaService.ingestWithSchema(
      this.schemaId,
      this.schemaName + '_' + new Date().toISOString().split('T')[0],
      data,
      ...
      (result) => {
        ...
        this.dataAnalyzing = false;
        this.stage = 'complete';
        // The backend decides whether the upload was appended to the current version, created a new
        // version, or reused an existing one — surface its explanation instead of a generic row count.
        ...
        // Make the just-uploaded dataset the active one across every page. Pass the schema name so
        // the correct family is activated even when the data was unchanged/deduped (no new row).
