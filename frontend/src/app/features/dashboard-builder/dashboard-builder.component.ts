...
export class DashboardBuilderComponent implements OnInit, OnDestroy {
  ...
  /** Only the active-schema dataset is offered in the picker; other datasets remain in the DB. */
  ...
  async confirmSave(): Promise<void> {
    ...
    try {
      ...
      // Saved widgets now live in "My Dashboards" — clear the canvas so the builder
      // starts fresh and only ever shows the current (unsaved) work.
      this.committedWidgets = [];
      this.widgetSeq = 0;
      this.startOver();
