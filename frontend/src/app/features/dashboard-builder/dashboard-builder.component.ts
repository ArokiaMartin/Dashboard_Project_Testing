...
export class DashboardBuilderComponent implements OnInit, OnDestroy {
  ...
  vizRows = [
    {
      cols: 3,
      groups: [
        { title: 'Single Metric', keys: ['kpi'] },
        { title: 'Detailed Data', keys: ['table'] },
        { title: 'Comparison', keys: ['bar', 'hbar'] }
      ]
    },
    {
      cols: 2,
      groups: [
        { title: 'Trend / Time Series', keys: ['line', 'area'] },
        { title: 'Part-to-Whole', keys: ['pie', 'donut'] }
      ]
    },
    {
      cols: 4,
      groups: [
        { title: 'Composition', keys: ['stacked'] },
        { title: 'Distribution', keys: ['scatter'] },
        { title: 'Multivariable Comparison', keys: ['radar'] },
        { title: 'Circular / Radial Magnitude', keys: ['polar'] }
      ]
    }
  ];

  getCardsInGroup(keys: string[]): VizCard[] {
    return (this.vizCards || []).filter(v => keys.includes(v.key));
  }
  ...
  private justStartedNameEdit = false;
  undoStack: WidgetSpec[][] = [];
  redoStack: WidgetSpec[][] = [];
  private widgetSeq = 0;
  ...
  private deactivate$ = new Subject<boolean>();
  saveMessage = '';
  ...
  showToast(msg: string, autoClear = true): void {
  ...
  ngOnInit(): void {
    const routeDashId = this.route.snapshot.paramMap.get('dashboardId');
    if (routeDashId) {
      this.pendingDashboardId = routeDashId;
      this.viewMode = true;
    } else {
  ...
  get filteredVisibleDatasets(): DatasetSummary[] {
    const q = this.dsSearch.toLowerCase().trim();
    if (!q) return this.visibleDatasets;
    const fuse = new Fuse(this.visibleDatasets, {
      keys: ['original_filename', 'table_name'],
      threshold: 0.4
    });
    return fuse.search(q).map((res: any) => res.item);
  }
  
  get selectedDatasetName(): string {
    const rowId = this.selectedDatasetRowId;
    const d = this.visibleDatasets.find((x) => x.id === rowId);
    return d ? this.datasetLabel(d) : 'Select dataset...';
  }

  get selectedDatasetRows(): number | null {
    const rowId = this.selectedDatasetRowId;
    const d = this.visibleDatasets.find((x) => x.id === rowId);
    return d ? d.row_count : null;
  }

  toggleDsDropdown(event: Event): void {
    event.stopPropagation();
    if (this.loadingDatasets) return;
    this.dsDropdownOpen = !this.dsDropdownOpen;
    if (this.dsDropdownOpen) this.dsSearch = '';
  }

  @HostListener('document:click')
  closeDsDropdown(): void {
  ...
  private tryLoadDashboardFromRoute(): void {
    ...
    this.dashboardService.getDashboardRecord(this.pendingDashboardId).subscribe({
      next: async (dashboard) => {
        ...
        this.showToast(`Loaded dashboard: ${dashboard.name}`);
      },
      error: () => {
        this.showToast('Could not load selected dashboard.');
      }
    });
  }
  ...
  private async activateDatasets(ids: string[], resetState: boolean): Promise<void> {
    ...
    const labels: string[] = [];
    for (const id of ids) {
      const data = await this.backend.getDatasetFlatData(id, 10000);
  ...
  private restoreWidget(index: number, widget: { chart_config_json?: Record<string, unknown>; database_config_json?: Record<string, unknown>; hydrated_data?: Record<string, unknown>[]; widget_name?: string; layout_json?: Record<string, unknown>; }): WidgetSpec | null {
    ...
    const base: WidgetSpec = {
      ...
      drillState: chart['drillState'] ? (chart['drillState'] as any) : undefined
    };
  ...
  colDisabledReason(c: Column): string {
    ...
    if (card) return `Doesn't fit ${card.label}`;
    return 'No chart works with this combination';
  }
  ...
  toggleColDropdown(event: Event): void {
    event.stopPropagation();
    this.colDropdownOpen = !this.colDropdownOpen;
    if (this.colDropdownOpen) this.colSearch = '';
  }

  getSelectedColsSummary(): string {
    if (this.selectedCols.length === 0) {
      return 'Select columns';
    }
    return this.selectedCols.map(c => c.name).join(', ');
  }
  ...
  filteredColsForDropdown(): Column[] {
    const q = (this.colSearch ?? '').toLowerCase().trim();
    const cols = this.compat.columns || [];
    if (!q) return cols;
    return cols.filter((c: Column) => c.name.toLowerCase().includes(q));
  }

  onColSearch(e: Event) {
    this.colSearch = (e.target as HTMLInputElement).value;
  }
  ...
  private cloneState(widgets: WidgetSpec[]): WidgetSpec[] {
    return widgets.map(w => ({
      ...w,
      layout: w.layout ? { ...w.layout } : undefined,
      databaseConfig: w.databaseConfig ? JSON.parse(JSON.stringify(w.databaseConfig)) : undefined,
      editState: w.editState ? JSON.parse(JSON.stringify(w.editState)) : undefined
    }));
  }

  saveStateForUndo(): void {
    this.undoStack.push(this.cloneState(this.committedWidgets));
  ...
  undo(): void {
    ...
    this.redoStack.push(this.cloneState(this.committedWidgets));
    const prevState = this.undoStack.pop()!;
    this.committedWidgets = prevState;
    this.syncDraft();
  }
  ...
  redo(): void {
    ...
    this.committedWidgets = nextState;
    this.syncDraft();
  }
  ...
  startEditingName(): void {
    ...
    this.justStartedNameEdit = true;
    setTimeout(() => {
      const input = document.querySelector('.db-title-input') as HTMLInputElement;
      input?.focus();
      input?.select();
      setTimeout(() => { this.justStartedNameEdit = false; }, 150);
    }, 0);
  }

  saveDashboardName(event: Event): void {
    if (this.justStartedNameEdit) return;
    const input = event.target as HTMLInputElement;
    const newName = input.value.trim();
    if (newName && newName !== this.loadedDashboardName) {
      this.saveStateForUndo();
      this.loadedDashboardName = newName;
      this.syncDraft();
      // Persist immediately to the backend if this is an existing dashboard
      if (this.pendingDashboardId) {
        this.saveDashboard();
      }
    }
  ...
  async editWidget(id: number): Promise<void> {
    ...
    if (!es) {
      this.showToast('This widget has no editable configuration.');
      return;
    }
    ...
    if (es.colNames.length && this.selectedCols.length < es.colNames.length) {
      this.showToast(`Editing "${widget.title}" — some columns couldn't be restored because their dataset is no longer available. Re-pick the columns, then click Update.`, false);
    } else {
      this.showToast(`Editing "${widget.title}" — change the columns/filters and click Update.`, false);
    }
  ...
  async saveDashboard() {
    ...
    if (this.pendingDashboardId) {
      ...
      const payload: SaveDashboardRequest = {
        user_id: 'anonymous',
        name: this.loadedDashboardName || `Dashboard ${new Date().toLocaleString()}`,
        description,
      ...
      try {
        await firstValueFrom(this.dashboardService.updateDashboardRecord(this.pendingDashboardId, payload));
        this.saveBusy = false;
        ...
        this.saveBusy = false;
        this.showToast('Error saving dashboard.');
      }
      return;
    }
  ...
  private async buildWidgetsPayload(): Promise<DashboardWidgetRecord[]> {
    ...
    if (this.visibleWidgets.length) {
      ...
      try {
        ...
        this.showToast('Cannot save: backend query failed for current widget.');
        return [];
      }
      widgetsToSave = [spec];
    }
  ...
  async confirmSave(): Promise<void> {
    ...
    try {
      ...
      this.syncDraft();
      this.startOver();
      this.router.navigate(['/dashboards'], { queryParams: { saved: 'true' } });
    } catch {
      this.saveBusy = false;
      this.showToast('Could not save dashboard. Please check the backend connection.');
    }
  }

  removeWidget(id: number) {
    this.saveStateForUndo();
    this.committedWidgets = this.committedWidgets.filter(w => w.id !== id);
    this.syncDraft();
  }
  ...
  // ---- CanDeactivate guard ----
  /** Called by the router guard. If there is nothing to save, allow navigation immediately.
   *  Otherwise show the custom modal and wait for the user to choose save or leave. */
  canDeactivate(): import('rxjs').Observable<boolean> | boolean {
    if (this.viewMode || !this.hasSomethingToSave()) return true;
    this.pendingDatasetId = null;  // navigation-triggered — not a dataset switch
    this.deactivate$ = new Subject<boolean>();
    this.showUnsavedModal = true;
    return this.deactivate$.asObservable();
  }

  // ---- unsaved-changes guard actions ----
  confirmLeave(): void {
    this.showUnsavedModal = false;
    this.undoStack = [];
    this.redoStack = [];
    if (this.pendingDatasetId) {
      // Dataset-switch flow — no router navigation needed
      ...
      this.deactivate$.next(false); // stay on builder page
      this.deactivate$.complete();
    } else {
      // Router-navigation flow — tell the guard to allow the navigation
      this.committedWidgets = [];
  ...
  toggleLabelDropdown(event: Event): void {
    ...
    if (this.labelDropdownOpen) this.chipSearch = '';
  }

  getSelectedLabelsSummary(): string {
    const active = this.activeLabels;
    const total = this.allLabelsForFilter().length;
  ...
  visibleChips(): string[] { return this.filteredChipList(); }
