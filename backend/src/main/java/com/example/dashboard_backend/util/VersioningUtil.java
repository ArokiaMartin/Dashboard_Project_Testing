...
public class VersioningUtil {
    ...
    /**
     * Calculate SHA-256 checksum from JsonNode (handles JSON data from requests).
     */
    public static String calculateChecksum(JsonNode jsonNode) {
        try {
            String jsonStr = objectMapper.writeValueAsString(jsonNode);
            return calculateChecksumFromString(jsonStr);
        } catch (Exception e) {
            logger.error("Error calculating checksum from JsonNode", e);
            return fallbackHash(jsonNode.toString());
        }
    }
